use druid::{
    commands as sys_cmd, commands, Color, Env, EventCtx, FileDialogOptions, FontDescriptor, FontFamily,
    ImageBuf, Point, Rect, Screen, Target, TextAlignment, UnitPoint, Vec2, Widget, WidgetExt, WidgetId, WindowDesc, WindowState,
};
use druid::widget::{
...
use crate::app_state::{AppState, State, ImageModified};
use crate::constants::*;
use crate::custom_widget::{
    ColoredButton, CustomSlider, CustomZStack, OverImages,
    ScreenshotImage, SelectedRect, StateShortcutKeys, TakeScreenshotButton,
    CREATE_ZSTACK, SAVE_OVER_IMG, SAVE_SCREENSHOT, SHOW_OVER_IMG, UPDATE_BACK_IMG,
    UPDATE_COLOR, UPDATE_RECT_SIZE, UPDATE_SCREENSHOT_CROP, UPDATE_SCREENSHOT_CROP_CLOSE,
};

pub fn build_about_us_widget() -> impl Widget<AppState> {
...
pub fn build_shortcut_keys_widget() -> impl Widget<AppState> {
...
pub fn build_screenshot_widget(monitor: usize) -> impl Widget<AppState> {
    ...
    .on_click(|ctx: &mut EventCtx, data: &mut AppState, _env: &Env| {
        ...
        if let Some(main_id) = data.main_window_id {
            ctx.submit_command(
                SAVE_SCREENSHOT
                    .with((
                        ...
                        main_id,
                        data.custom_zstack_id.unwrap_or(WidgetId::reserved(0)), // Fallback or handle error
                        data.screenshot_id.unwrap_or(WidgetId::reserved(0)),
                        base_path,
                        name,
            ...
            eprintln!("Main window ID is missing!");
        }
...
pub fn build_screenshot_crop_widget(monitor: usize) -> impl Widget<AppState> {
    let rectangle = LensWrap::new(SelectedRect::new(monitor), AppState::rect);
...
pub fn build_colors_window(zstack_id_param: WidgetId) -> impl Widget<AppState> {
        ...
        .on_click(move |ctx: &mut EventCtx, data: &mut AppState, _env: &Env| {
            if let Err(e) = ctx.get_external_handle()
                .submit_command(UPDATE_COLOR, (color, None), Target::Widget(zstack_id_param)) {
                    eprintln!("Error submitting command: {:?}", e);
                }
            data.color = color;
            ctx.window().close();
        })
